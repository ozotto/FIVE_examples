...
(function() {
    ...
    function configureMap(){
        ...
        timeMap.projection = d3.geo.albers()
            ...
            .parallels([100, 100])
            .scale(3000)
            .translate([(timeMap.options.graph.width / 2) + 50, (timeMap.options.graph.height -timeLineHeight) / 3 ]);
    ...
    function createMap(error, data1, data2, data3){
        ...
        if(timeMap.svg.country != null ){
            ...
            country = topojson.feature(data1, data1.objects.germany).features;
        }

        if(timeMap.svg.state != null ){

            if(timeMap.svg.country != null )
                state = topojson.feature(data2, data2.objects.statesGermany).features; else
